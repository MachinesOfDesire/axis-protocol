...
- **§8 Step 1.3 — claim-validation list reconciled with §4.3.** The step listed `axis_version` and `registry_url` alongside the required claims as if all were mandatory, contradicting §4.3 (both OPTIONAL; `registry_url` advisory) and §4.3.2 (`axis_version` is a v0.4 addition that no shipped issuer emits — verifiers MUST NOT reject a token solely for lacking it). The step now names the required claims (`iss`, `aud`, `iat`, `exp`) and validates the optional ones only when present.

### Editorial

- **§4.1, §4.2, §4.4 — `axis_version` example values aligned with §4.3.2.** The AIR field description still said "e.g. `\"0.1\"`" against a `"0.2"` example; the OIR example carried `"0.1"` though registry-generated records carry the current record-format version (`"0.2"`, per §4.3.2); the §4.4 DC example carried `"0.1"` while showing the v0.2 `proofType: "jcs-eddsa-2026"` proof. Examples and the field description now show `"0.2"`. Illustrative only — no data-model change.
- **ROADMAP.md — staleness banner and corrected cross-references.** The narrative roadmap still presented v0.2 and v0.3 as "planned," sketched a pre-v0.3 scope vocabulary that conflicts with the shipped Appendix B (different strings; bare platform prefixes instead of the `x-<vendor>:` rule), and linked to SPEC.md §17.1/§17.2/§17.3 anchors that no longer exist. Added a status banner establishing SPEC.md §17 + this CHANGELOG as authoritative, a shipped-in-revised-form note on the scope-vocabulary sketch, and fixed the anchors. The narrative text itself is retained as design rationale.
