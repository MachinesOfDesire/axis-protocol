...
- **§4.3 / §4.3.2 — AIT `axis_version` claim now shipping issuer-side.** The reference hosted issuer now stamps `axis_version` (the protocol version the token was minted against) on every AIT. §4.3.2 moves the claim's issuer-side emission from "specified for v0.4 — not yet shipped" to "shipped today"; the optional-claims entry in §4.3 is updated to match. The change is purely additive: shipped verifiers read only the claims they gate on and ignore `axis_version` (they MUST NOT reject a token for carrying or lacking it), and the claim is covered by the AIT signature like any other payload field. Verifier-side enforcement/negotiation (and the general issuer SHOULD) remain specified for v0.4. No normative requirement changes — this stamps the most-frozen wire artifact ahead of v0.4 so future format changes can be negotiated without a flag day.
