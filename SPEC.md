...
- `axis_version` — protocol version signaling: the protocol version the issuer minted the token against. **Shipped (issuer-side):** the reference hosted issuer emits this claim on every AIT. It is additive and forward-compatible — shipped verifiers ignore it and MUST NOT reject a token for carrying (or lacking) it. Verifier-side enforcement/negotiation is specified for v0.4. See §4.3.2.
...
- **AIT `axis_version` claim (issuer-side)** — the protocol version the issuer minted the token against. The reference hosted issuer now emits `axis_version` on every minted AIT. The claim is purely additive and forward-compatible: shipped verifiers read only the claims they gate on (§6.4, §8) and ignore `axis_version`, so a token carrying it verifies exactly as one without it, and it is covered by the AIT signature like any other payload claim. Stamping it now — on the most-frozen artifact every platform parses — is what lets a future format change be negotiated without a flag day. Verifiers MUST NOT reject a token merely for carrying (or lacking) this claim.

**Specified for v0.4 — not yet shipped:**

- **AIT `axis_version` verifier enforcement** — no shipped verifier checks the claim today. From v0.4, verifiers MAY act on it: a verifier MUST NOT reject a token solely for lacking `axis_version` until its advertised minimum-supported version (see `/.well-known/axis-versions`, §6.16) is v0.4 or later; a verifier MAY reject a token that *declares* a version below its minimum-supported version. From v0.4, all issuers SHOULD emit it on every AIT (the reference hosted issuer already does — see above).
